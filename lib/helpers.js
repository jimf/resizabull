 ...
 * @param {Number} [val] Value to use instead of resizer.x
 * @return {number} Clamped width value
 */
exports.clampWidth = function(resizer, val) {
    return clamp(val || resizer.x, resizer.minWidth, resizer.maxWidth);
};
 ...
 */
exports.clampHeight = function(resizer, val) {
    return clamp(val || resizer.y, resizer.minHeight, resizer.maxHeight);
};


/**
 * Return a value based on the resizer's `aspectRatio`.
 *
 * @param {Object} resizer Resizer object
 * @param {Number} [val] Value to use instead of resizer.x
 * @return {Number} Height value that maintain's the aspect ratio
 */
exports.preserveHeightRatio = function(resizer, val) {
    if (resizer.aspectRatio === false) { return val || resizer.y; }
    return (val || resizer.x) / resizer.aspectRatio;
};
 ...
 *
 * @param {Object} resizer Resizer object
 * @param {Number} [val] Value to use instead of resizer.y
 * @return {Number} Width value that maintain's the aspect ratio
 */
exports.preserveWidthRatio = function(resizer, val) {
    if (resizer.aspectRatio === false) { return val || resizer.x; }
    return (val || resizer.y)  * resizer.aspectRatio;
};
