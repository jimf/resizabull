...
function renderResizer(resizer) {
    ...
    if (grab.onRightEdge) {
        newWidth = helpers.clampWidth(resizer);
        newHeight = helpers.clampHeight(
            resizer, helpers.preserveHeightRatio(resizer, newWidth));
    }

    if (grab.onBottomEdge) {
        newHeight = helpers.clampHeight(resizer);
        newWidth = helpers.clampWidth(
            resizer, helpers.preserveWidthRatio(resizer, newHeight));
    }
